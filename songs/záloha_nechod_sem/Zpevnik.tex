...
\usepackage[a5paper, top = 0.7cm, left = 1cm, right = 1cm, bottom = 1.5cm]{geometry} % aby byly mensi okraje
...
\usepackage{multicol}
\usepackage{graphicx}
\usepackage{hyperref} %tvoří interaktivní obsah - kliknutím se dostaneš na danou písničku
...
%  \fi
%}
\newcommand*\doublepage{\cleardoublepage}
...
\author{Martin a Mara}%je vidět jen zde
\title{Zpěvník na Vánočku 2019} %název se taky nikde nezobrazuje, ale pro pořádek
...
\setcounter{page}{2}  % nastaví čítač stránek znovu od jedné a můžeme začít vkládat písně
% většina chyb, ktré vzniknou souvisí s vkládáním obrázků - některé písně v sobě mají vložený obrázek, ale ten nenajdou protože tam není.
% některá varování a bad boxy jsou způsobena sazbou písní (prázdné odstavce atd.). Nikdy nejsou fatální, ale možná s tím půjde něco udělat.
% překlad se musí nastavit na tex -> ps -> pdf

%\input{songs//koledy/_seznam2.txt} % možnost přidání celého textového souboru s písněmi - více písní najednou
\newpage

\pagestyle{empty}
\tableofcontents %tady se generuje automatický obsah, tady se dá měnit. Správně se vygeneruje vždy až při druhém překladu.

\add{akordy.tex} % jednotlivé písničky
